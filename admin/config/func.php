...
{	global $root_dir;
	echo "<script>alert('".$title."');document.location='".$root_dir."admin/index.php?cmd=".$cmd."&act=".$act."'</script>";
}

function upload_file($filename)
{
global $upload_dir,$max_file_size_upload,$small_dir,$small_width,$small_height,$thumb_dir,$thumb_width,$thumb_height;
...
for($i=0;$i<count($_FILES[$filename]["name"]);$i++)
{
	...
	{	$newfilename=$i."_".time().'.'.$imageFileType;
		if (move_uploaded_file($_FILES[$filename]["tmp_name"][$i], $target_dir.$newfilename)) 
		{	
			createThumbnail($target_dir.$newfilename, $small_dir.$newfilename, $small_width, $small_height,["255","255","255"]);
			createThumbnail($target_dir.$newfilename, $thumb_dir.$newfilename, $thumb_width, $thumb_height,["255","255","255"]);
...
function upload_file1($filename)
{
global $upload_dir,$max_file_size_upload,$small_dir,$small_width,$small_height,$thumb_dir,$thumb_width,$thumb_height;
...
$uploadOk = 1;
	...
	{	$newfilename=time().'.'.$imageFileType;
		...
		{	
			createThumbnail($target_dir.$newfilename, $small_dir.$newfilename, $small_width, $small_height,false);
...
function upload_color($filename)
{
	$all_color="";
	$all_color.=$filename."|";
	return $all_color;
...
function img_display($name,$width=50,$stt=0,$number=0)
{
	global $upload_dir;
	if($number==0)
	{
		...
		{
			if($i==$stt)
			$rs="<img src='".$upload_dir.$each_mig_name[$i]."' width='".$width."' alt='...'>";
			elseif($stt<0)
			$rs.="<img src='".$upload_dir.$each_mig_name[$i]."' width='".$width."' alt='...'>";
		}
		return $rs;
	...
	{
		$rs="<img src='".$upload_dir.$name."' width='".$width."' alt='...'>";
		return $rs;
	}
}
